...
export default function CodeCard({ code, lockedPlatform }: CodeCardProps) {
  ...
  return (
    <motion.div
      ...
      className="nb-card-cyan nb-press p-4 sm:p-5"
    >
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="text-xs px-2 py-0.5 rounded-none bg-sunset-orange/10 text-sunset-orange border-2 border-sunset-orange shrink-0 font-semibold">
            {code.effect}
          </span>
        </div>
          ...
          <div className="flex gap-1.5">
            {(["PS5", "Xbox", "PC"] as const).map((p) => (
              <button
                ...
                className={`text-xs px-2.5 py-1 rounded-none font-medium transition-all ${
                  platform === p
                    ? platformStyles[p]
                    : "bg-deep-bg-light text-text-muted border-2 border-night-violet/50 hover:text-text-secondary"
                }`}
              >
                {p}
              </button>
        ...
        <div className="flex items-center gap-2">
          <code className="cheat-code flex-1 block px-3 py-2 rounded-none bg-deep-bg border-2 border-lagoon-cyan/40 text-center">
            {codeValue}
          </code>
          <button
            onClick={handleCopy}
            className={`shrink-0 p-2 rounded-none border-2 transition-all ${
              copied === platform
