...
export default function GameCard({ title, description, href, icon, image, accent = "pink", stats }: GameCardProps) {
  ...
  return (
    ...
    >
      ...
      {!image && icon && (
        <div className={`h-40 sm:h-48 flex items-center justify-center ${colors.icon}`}>
          <span className="text-5xl">{icon}</span>
        </div>
      )}
      <div className="p-4 sm:p-5">
        <h3 className={`font-semibold text-text-primary group-hover:${colors.text} transition-colors`}>
          {title}
        </h3>
